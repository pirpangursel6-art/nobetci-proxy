...
app.get("/api/places", rateLimit, async (req, res) => {
  ...
  try {
    ...
    if (shaped.length === 0) {
      // Son çare: ne OSM ne Open Places bir şey bulamadı — elle
      // doğruladığımız bir yedek kayıt varsa (bkz. MANUAL_SEED_PLACES),
      // en azından onu gösterelim, tamamen boş bırakmak yerine.
      shaped = runManualSeeds(category, lat, lng, radius, limit);
    }
    if (shaped.length === 0 && category === "itfaiye") {
      // İtfaiye için son çarelerin de son çaresi: 81 ilin resmi itfaiye
      // adresinden (bkz. PROVINCE_FIRE_STATION_ADDRESSES), kullanıcının
      // bulunduğu ile göre geocode edilmiş tek bir sonuç.
      shaped = await runProvinceFireStationFallback(lat, lng, radius);
    }
    // Sadece "kampüs" tarzı kurumlarda (aynı hastane/belediyenin farklı
    // isimli bölümleri gibi) uyguluyoruz — market, eczane, cafe gibi
    // yoğun kategorilerde iki farklı işletme gerçekten 150 metre içinde
    // yan yana olabilir, orada bu mantık yanlışlıkla gerçek/farklı bir
    // sonucu gizlerdi (yakın bir eczanenin kaybolması gibi).
