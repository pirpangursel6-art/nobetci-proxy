...
const CATEGORY_CONFIG = {
  ...
  kahvehane: { query: "kahvehane kıraathane nargile çay ocağı", type: "cafe" },
  ...
  metro_tramvay: { query: "metro tramvay otobüs vapur taksi durağı", type: "transit_station" },
  ...
  burger: { query: "burger hamburger döner pide lahmacun", type: "restaurant" },
  yedek_parca: { query: "oto yedek parça", type: null },
  tursu_aktar: { query: "turşucu aktar baharatçı çerezci kuruyemişçi", type: null },
  spor_salonu: { query: "spor salonu fitness halı saha", type: "gym" },
  ...
  poliklinik: { query: "özel poliklinik muayenehane", type: "doctor" },
  ...
  mobilyaci: { query: "mobilyacı beyaz eşya mağazası", type: "furniture_store" },
  ...
  avukat: { query: "avukat hukuk bürosu noter", type: "lawyer" },
  sigorta: { query: "sigorta acentesi", type: "insurance_agency" },
  okul: { query: "okul anaokulu kreş", type: "school" },
...
function cacheKeyFor(category, lat, lng, radius, limit) {
  ...
  return `${category}:${rLat}:${rLng}:${radius}:${limit}`;
}
...
app.get("/api/places", rateLimit, async (req, res) => {
  ...
  // 50km rather than 10km — still a hard cutoff (see haversineMeters
  // above), just a wider one. `limit` lets the frontend ask for more
  // results when it's only fetching one category (a person browsing
  // "Eczane" wants to see everything within 50km, not just 6 of them) —
  // it stays low by default because "Yakınımdakiler" fires every
  // category in parallel, and a high limit there for all of them at
  // once would recreate the slow-load problem we already fixed.
  ...
  const key = cacheKeyFor(category, lat, lng, radius, limit);
  ...
  try {
    ...
    // round-trip (phone, full weekly hours, photos), and doing that for
    // all 20 when someone opens a category is what made the list slow to
    // show up. Only the closest ENRICH_COUNT get that treatment up
    // front; the rest are shaped from Text Search's own fields alone —
    // still a real, useful entry (name, address, rating, distance), just
    // without phone/full hours/photos until the person actually taps
    // into it (see /api/place-detail below, which the app calls lazily
    // at that point).
