...
// Runs several source lookups at once and resolves with whichever returns
// real (non-empty) results first, instead of trying them one after
// another — this is what actually cuts the wait, since a slow or blocked
// source no longer has to fully time out before the other one even gets
// a chance to answer.
...
// Aynı gerçek hastane, Overture'ın veri setinde birden fazla kayıt olarak
// bulunabiliyor — resmi tam ismi ("... Mustafa Kirazoğlu Devlet
// Hastanesi") ile halk arasındaki kısa ismi ("... Devlet Hastanesi"), ya
// da bir hastanenin ayrı bir bölümü/servisi ("... Yoğun Bakım Servisi")
// ayrı bir "yer" gibi kaydedilmiş olabiliyor. İsimler birbirinden farklı
// olduğu için isme göre tekilleştirme bunları yakalayamıyordu. Bunun
// yerine: konumları birbirine çok yakınsa (150 metre içinde), muhtemelen
// aynı hastane kampüsü/binasıdır — isme bakmaksızın sadece en yakın
// (listede zaten mesafeye göre sıralı olduğu için ilk geçen) kayıt
// tutuluyor.
